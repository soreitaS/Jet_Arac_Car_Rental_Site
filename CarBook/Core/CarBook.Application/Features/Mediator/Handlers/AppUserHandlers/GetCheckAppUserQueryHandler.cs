...
namespace CarBook.Application.Features.Mediator.Handlers.AppUserHandlers
{
    public class GetCheckAppUserQueryHandler : IRequestHandler<GetCheckAppUserQuery, GetCheckAppUserQueryResult>
    {
        ...
        public async Task<GetCheckAppUserQueryResult> Handle(GetCheckAppUserQuery request, CancellationToken cancellationToken)
        {
            ...
            var user = await _appUserRepository.GryByFilterAsync(x => x.Username == request.Username && x.Password==request.Password);
            if(user == null)
            {
            ...
            else
            {
                ...
                values.Role = (await _appRoleRepository.GryByFilterAsync(x => x.AppRoleId == user.AppRoleId)).AppRoleName;
